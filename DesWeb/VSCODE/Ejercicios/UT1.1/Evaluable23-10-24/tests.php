...
<body class="container">
    ...
    echo "<h2>Todos los usuarios</h2><ul>";
        $users = $calendarDataAccess->getAllUsers();
        foreach ($users as $user) {
            echo "<li>ID: {$user->getId()}, Nombre: {$user->getFirstName()} {$user->getLastName()}, Email: {$user->getEmail()}, Pass: {$user->getPassword()}, Birth: {$user->getBirthDate()}, About: {$user->getAbout()}</li>";
        }
        echo "</ul>"; 
